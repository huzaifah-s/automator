...
const USAGE = `Usage: bun run secret -- <command>
  ...
  list                   Every stored credential, names only
...
function list(): number {
  ...
  const updated = new Map(store.secretRows().map((r) => [r.key, r.updated_at]));
  for (const key of keys) {
    const at = updated.get(key);
    console.log(`${key.padEnd(34)} ${at ? new Date(at).toISOString() : ""}`);
  }
  console.log(`\n${keys.length} stored secret(s). Values are never printed by list.`);
  return 0;
}
